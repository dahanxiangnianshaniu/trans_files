...
**适用范围**：本降级策略覆盖所有阶段，包括新增的阶段四D（留存期合规性深度检查）。4D步骤与现有阶段保持一致的降级策略。
...
**数据控制者模式追加关键词**：个人数据清单、个人数据列表、DI清单、personal data list、个人数据留存规范、留存规范、retention standard

**数据控制者模式追加Glob扫描模式**：`**/*个人数据清单*.*`、`**/*个人数据列表*.*`、`**/*DI清单*.*`、`**/*personal*data*list*.*`、`**/*留存规范*.*`、`**/*retention*standard*.*`

**注意**：设备供应者模式下不追加DI清单和留存规范的文件名搜索关键词。3轮搜索后的中止判断仅基于核心文档（数据控制者：隐私声明；设备供应者：个人数据说明）。DI清单和留存规范属于补充文档，未找到时不触发整体中止。

### 第2轮 — 扩展搜索

用 `Grep` 在所有文件内容中搜索扩展关键词（隐私/声明/政策/个人信息/用户信息/数据收集/数据说明/privacy/data/personal/information/collect/statement）→ 收集命中文件列表

**数据控制者模式追加关键词**：留存、retention、保存期限
...
1. **Read 优先**：尝试使用 Read 工具直接读取 xlsx 文件（opencode 的 Read 工具支持 xlsx 格式，可获取页签名和列值）
2. **Bash+Python 兜底**：Read 无法满足结构化提取需求时，使用 Bash+Python 脚本（openpyxl 读取 xlsx），与现有降级策略一致。脚本模板见上方"xlsx 读取"章节

### 留存规范xlsx读取分层策略

与DI清单相同：Read 优先 → Bash+Python 兜底

### 4D-4删除能力关键词扫描的Grep+Read替代方案

**问题**：Grep 对二进制文件（xlsx/chm等）可能无法命中关键词

**替代方案**：

1. 先用 Grep 在文本类文件中搜索关键词
2. 对 Grep 无命中的二进制文件，使用 Read 工具逐文件读取内容后由 LLM 判断是否包含删除能力描述
